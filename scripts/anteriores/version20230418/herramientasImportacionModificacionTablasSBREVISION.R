...
eliminarFilasCODIGOnaCUENTAna <- function(tabla) {
  ...
  if ( all(c("CODIGO","CUENTA") %in% names(tabla)) ) {
    ...
    # Determinamos expresiones regulares para texto
    expresion_regular_texto <- "[[:alpha:]]"
    # Determinamos la prueba lógica para determinar la fila donde la columna CODIGO es texto o NA y la columna CUENTA es NA
    prueba <-
...
eliminarFilasCODIGOnaCUENTAcod <- function(tabla) {
  ...
  if ( all(c("CODIGO","CUENTA") %in% names(tabla)) ) {
    ...
    expresion_regular_codigo <- "^[[:digit:]]{1,6}$"
    # Determinamos la prueba lógica para determinar la fila donde la columna CODIGO NA y la columna CUENTA es un código de 6 dígitos
