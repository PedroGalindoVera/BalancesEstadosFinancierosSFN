...
identificarFechaCorteBoletinSB <- function(tabla) {
  ...
  return( as.Date(fecha_corte_date[1]) )
}
...
compiladorHojasBalanceFinancieroSBP <- function(nombre_hoja_buscada = "BALANCE") {
  ...
  lista_hojas <- listaImportacionDatosFuenteSB(nombre_hoja_buscada = nombre_hoja_buscada)
  # Fundido (melting) de tablas
  lista_tablas <- lapply(lista_hojas , function(tabla)
