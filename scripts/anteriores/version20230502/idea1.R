lista_libros <- list.files(destino, recursive = TRUE)
unique(tools::file_ext(lista_libros))


ruta_directorio <- "data/Fuente/SB/PUBLICA"
...
hoja_libro <- "BALANCE"
fila_fin = 30
hoja <- importarLibroExcel(ruta_libro, hoja, fila_fin) 

#hoja <- readxl::read_excel(ruta_libro, sheet = "BALANCE", n_max = 20)
fecha_corte <- analisisDifusoNLPFechaCorte(hoja)

hojaToTablaBoletinesFinancierosSB(ruta_libro, nombre_hoja)
...
hojaToTablaBoletinesFinancierosSB <- function(ruta_libro, nombre_hoja, fecha_corte = NULL) {
  ...
  hoja <- importarLibroExcel(ruta_libro, nombre_hoja, 30) 
  ...
  tabla_prueba <-
    importarLibroExcel(ruta_libro, nombre_hoja, 20, indice_fila_nombres_columnas) #%>% 
    #select_if(~ !all(is.na(.)))
  ...
  if ( mean(nombres_columnas == names(tabla_prueba), na.rm = TRUE) < 0.8 ) {
    ...
    while ( mean(nombres_columnas == names(tabla_prueba), na.rm = TRUE) < 0.8 & indice_fila_nombres_columnas <= 20 ) {
      ...
      tabla_prueba <- importarLibroExcel(ruta_libro, nombre_hoja, 20, fila_inicio = indice_fila_nombres_columnas) 
    }
  }
