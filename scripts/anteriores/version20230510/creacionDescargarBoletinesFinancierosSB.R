descargarBoletinesFinancierosSB <- function(ruta_archivo_html) {
  ...
  if ( prueba_char ) {
    ...
    nombre_carpeta <- gsub(".zip", "", names(download_links_elegidos))
    nombre_carpeta <-
      unlist(
        regmatches(carpeta, gregexpr(expresion_regular_anios, nombre_carpeta)))
  ...
  for (k in seq_along(download_links_elegidos) ) {
    ...
    if ( prueba_anio ) {
      directorio_carpeta <-
        gsub(basename(directorio_carpeta),nombre_carpeta[k],directorio_carpeta)
    }
    if ( !dir.exists(directorio_carpeta) ) crearDirectorio(directorio_carpeta)
    #----
