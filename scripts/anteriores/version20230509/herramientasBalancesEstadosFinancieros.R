...
analisisVinculosPaginaWebSEPS <- function() {
  ...
  cat("\nBuscando enlaces de descarga en la página: [",link,"]\n")
...
descargaArchivosEnlacesAnalizados <- function(enlaces, informacion, ruta_destino) {
  ...
  for ( k in seq_along(url) ) {
    ...
    if ( (!file.exists(dest_file) || grepl(format(Sys.Date(), "%Y"),url[k]) ) && status[k] == 200 ) {
      ...
      cat("\nEl archivo: [", basename(url[k]), "] ya existe en el directorio: [", dest_file, "].\n")
...
concatenarTablasSEPS <- function() {
  ...
  attributes(tabla_concatenada) <- NULL
...
modificadorNombresColumnasTablasIF <- function(tabla, catalogo = NULL) {
  ...
  analisis_nombres_columnas <-
    analisisTextoSimilar(nombres_columnas, nombres_catalogo, precision = 0.85)
  ...
  cat("Nombres de columnas modificados:\n")
...
unionBalanceFinancieroSB <- function() {
  ...
  data.table::fwrite(consolidada, nombre_archivo)
  cat("\nSe ha creado el archivo con la ruta: [", normalizePath(nombre_archivo), "]\n")
  cat("\n Duración:", difftime(Sys.time(), tic, units = "mins"), "minutos." )
