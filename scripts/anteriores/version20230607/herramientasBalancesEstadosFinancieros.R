  ...
  for (enlace in enlaces) {
    ...
    nueva_fila <-
      data.frame(
        ...
        url = response$url,
        filename =
          ifelse(length(filename_temporal) > 0, filename_temporal, NA),
...
fusionarBalancesSBEstadosSEPSFinancieros <- function() {
  ...
  SB <- crearBalancesFinancierosSB() # Verificado en prueba individual 2023/05/15
