...
exportarProyecto <- function() {
  ...
  if ( dir.exists(ruta_directorio_compartido) ) {
    copiarProyecto(ruta_directorio_compartido)
  }
}
...
gestorDescargasDescompresionSEPS <- function() {
  ...
  info_enlaces_SEPS <- obtenerEnlacesDescarga(enlaces_descarga = enlaces_SEPS, identificador = "SEPS")
...
modificarNombreColumnaSB <- function(tabla, precision = NULL, catalogo = NULL) {
  ...
  if ( is.null(catalogo) ) {
    ...
    catalogo <- readxl::read_excel("data/Catalogos/Catálogo Operadores.xlsx")
...
agregarRUCenSB <- function(tabla, ruta_catalogo = NULL) {
  ...
  if ( is.null(ruta_catalogo) ) {
    ruta_catalogo <- "data/Catalogos/Catálogo Operadores.xlsx"
  }
  precision <- 0.9
...
crearBalancesFinancierosSB <- function() {
  ...
  ejecutarDescargaBoletinesFinancierosSB() # Verificado en prueba individual 2023/05/11, crear una alternativa recurciva para error de descarga
  ...
  descomprimirArchivosDirectorioZip(origen, destino) # Verificado en prueba individual 2023/05/11
...
fusionarBalancesSBEstadosSEPSFinancieros <- function() {
  ...
  SB <- crearBalancesFinancierosSB() # Verificado en prueba individual 2023/05/12
  SEPS <- crearEstadosFinancierosSEPS() # Verificado en prueba individual 2023/05/12
  BEF <- rbind(SEPS, SB)
