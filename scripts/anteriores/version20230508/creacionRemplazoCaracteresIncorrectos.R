...
correcionCaracteresParalelizada <- function(vector_texto) {
  ...
  texto_corregido_parelizado <-
    parallel::mclapply(
      bloques_texto,
      function(bloque) stringr::str_replace_all(bloque, correcciones))
  texto_corregido <- unlist(texto_corregido_parelizado)
  return(texto_corregido)
}

texto <- head(SEPS$RAZON_SOCIAL,507000)
t <- analisisCaracteresIncorrectos(texto)
v <- correcionCaracteresParalelizada(texto)

grep("  ", unique(texto_corregido), value = TRUE)
grep("Ãš", unique(texto_corregido), value = TRUE)
analisisCaracteresIncorrectos(texto_corregido)
