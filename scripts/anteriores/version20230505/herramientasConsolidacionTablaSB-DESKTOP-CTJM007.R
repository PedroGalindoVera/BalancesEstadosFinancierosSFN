...
# correccionCaracteresParalelizadaRazonSocialSEPS <- function(tabla) {----
#   
  ...
  for ( archivo in archivos ) {
    ...
    lista_tablas_SEPS[[nombre_tabla]] <-
      ...
      dplyr::mutate(`FECHA` = lubridate::ymd(`FECHA`),
                    ...
                    #`RAZON_SOCIAL` = stringr::str_replace_all(`RAZON_SOCIAL`,correciones_RAZON_SOCIAL)
      ) #%>%
      # dplyr::mutate(`RAZON_SOCIAL` = stringr::str_replace_all(`RAZON_SOCIAL`,
      #   c("Ã‘" = "Ñ", "Ã‰" = "É", "Ã\u008d" = "Í", "Ã“" = "Ó")))
  }
...
#     prueba_extension_xls <- grepl("\\.xls", ruta_origen)
#     if ( !dir.exists(ruta_destino) ) dir.create(ruta_destino)
#     #if ( tools::file_ext(archivo) == "zip" ) {
#     if ( prueba_extension_zip & !prueba_zip_contenidos ) {
